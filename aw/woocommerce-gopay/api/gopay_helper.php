...
class GopayHelper {
    ...
    */
  	public static function concatRefundRequest($targetGoId,
  		...
  		return $targetGoId . "|" . $paymentSessionId . "|" . $amount . "|" . $curency . "|" . $description . "|" . $secureKey;
	}
	...
	public static function checkPaymentStatus($paymentStatus,
		...
		if (! empty($paymentStatus)) {
			...
			if ( $paymentStatus->result != GopayHelper::CALL_COMPLETED) {
                                if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: checkPaymentStatus - result - '.$_SERVER['HTTP_HOST'],  "nothing to say");
                                      } 
                                }  
			...
			if ( $paymentStatus->sessionState != $sessionState) {
                                if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: checkPaymentStatus - sessionState - '.$_SERVER['HTTP_HOST'],  "nothing to say");
                                      } 
                                } 
				throw new Exception("PS invalid session state");
			}

			if (trim(str_replace("#", "", $paymentStatus->orderNumber)) != trim(str_replace("#", "", $orderNumber))) {
                                if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: checkPaymentStatus - orderNumber - '.$_SERVER['HTTP_HOST'],  "nothing to say");
                                      } 
                                }
				throw new Exception("PS invalid VS -> ".$paymentStatus->orderNumber." != ".$paymentStatus->orderNumber);
			}

			if (trim($paymentStatus->productName) != trim($productName)) {
                                 if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: checkPaymentStatus - productName - '.$_SERVER['HTTP_HOST'],  "nothing to say");
                                      } 
                                 }
			...
			if ($paymentStatus->targetGoId != $goId) {
                                if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: checkPaymentStatus - targetGoId - '.$_SERVER['HTTP_HOST'],  "nothing to say");
                                      } 
                                 }
			...
			if ($paymentStatus->totalPrice != $totalPriceInCents*100) {
                                if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: checkPaymentStatus - totalPrice - '.$_SERVER['HTTP_HOST'],  $paymentStatus->totalPrice.' != '.$totalPriceInCents*100);
                                      } 
                                 }
				throw new Exception("PS invalid price -> ".$paymentStatus->totalPrice.' != '.$totalPriceInCents*100);
			}

			if ($paymentStatus->currency != $currency) {
                                if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: checkPaymentStatus - currency - '.$_SERVER['HTTP_HOST'],  $paymentStatus->currency."!=".$currency);
                                      } 
                                 }
				throw new Exception("PS invalid currency");
                        ...
                        if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: checkPaymentStatus - prazdny result - '.$_SERVER['HTTP_HOST'],  "nothing to say");
                                      } 
                        }
		...
		if ($decryptedHash != $hashedSignature) {
                        if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: checkPaymentStatus - decrypt invalid signature - '.$_SERVER['HTTP_HOST'],  "nothing to say");
                                      } 
                        }
	...
	public static function checkPaymentIdentity($returnedGoId,
		...
		if (trim(str_replace("#", "", $returnedOrderNumber)) != trim(str_replace("#", "", $orderNumber))) {
			throw new Exception("PI invalid VS -> ".trim(str_replace('#', '', $orderNumber)).' != '.trim($returnedOrderNumber));
		}
