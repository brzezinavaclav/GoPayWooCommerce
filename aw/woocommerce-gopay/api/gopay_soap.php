...
class GopaySoap {
	...
	public static function isPaymentDone($paymentSessionId,
	 	...
	 	try {
			...
			if ( $paymentStatus->result != GopayHelper::CALL_COMPLETED) {
                                if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: isPaymentDone - invalid status CALL_COMPLETED - '.$_SERVER['HTTP_HOST'],  print_r($paymentStatus,true));
                                      } 
                                }                           
			 ...
			 */
                        if ($this->get_option('gopay_enviroment_debug') == 'yes') {
                                      if (($this->get_option('gopay_enviroment_email')) != "") {
                                          wp_mail( $this->get_option('gopay_enviroment_email'), 'CHYBA: isPaymentDone - '.$_SERVER['HTTP_HOST'],  print_r($f,true));
                                      } 
                        }
			throw new Exception("Communication with WS failed");
		}
	 ...
	 * @param string $recurrentPaymentCurrency
	...
	public function refundPaymentPartially($paymentSessionId,
		...
		try {
			...
			$refundRequest =  array(
		 	...
		 	$paymentResult = $go_client->__call("refundPayment", array("refundRequest"=>$refundRequest));
